...
В той задаче вам предстояло вывести информацию о питомце на экран. Сейчас вам необходимо создать словарь pets = 0}
...
Задача не будет считаться выполненной, если вы заходите сразу внести информацию, не прибегая в функции input Например:
pets = (
"Мухтар": (
"Вид питомца": "Собака",
"Возраст питомца": 9,
...
Его необходимо заполнить пользовательским вводом через консоль с помощью функции inputO, 
...
Его возраст: 21 год
Его возраст: 19 лет
И теперь осталось только получить всю информацию о питомце в виде строки, как из задания по Урок Nº3. 
Ввод-вывод и базовые переменные. Задание Nº1, но с небольшими изменениями. 
...
while True:
    ...
    another = input("Хотите добавить еще одного питомца? (да/нет): ")
    if another.lower() != "да":
        break
